...
class DGCNTransformer(CaptioningModel):
    def __init__(self, model: Transformer, weight_files):
        super(DGCNTransformer, self).__init__()
        self.n = len(weight_files)
        self.models = ModuleList([copy.deepcopy(model) for _ in range(self.n)])
        for i in range(self.n):
            state_dict_i = torch.load(weight_files[i])['dict']
            self.models[i].load_state_dict(state_dict_i)

    def step(self, t, prev_output, visual, seq, mode='forcing', **kwargs):
        out_ensemble = []
        for i in range(self.n):
